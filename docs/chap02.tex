...
\begin{figure}[h] 
	\centering
	\includegraphics[width=0.8\textwidth]{image/sponge-celltype.jpg}
	% \bicaption{图片插入示例}{Example of inserting figures}
	\bicaptionX{复杂图片注释示例}{Example of complex image annotation}{（\textbf{A}）动物祖先系统发育树。（\textbf{B}）\textit{S.lacustris}鸟瞰图。（\textbf{C}）10106 个细胞组成的 TSNE 图，颜色表示42个不同聚类。（\textbf{D}）PAGA 连接图，边的粗细代表连接强度，节点尺寸代表细胞数量。（\textbf{E}） PAGA 路径的差异表达标记的归一化表达值。}
	\label{fig:sponge}
\end{figure}
